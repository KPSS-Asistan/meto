...
(function () {
    ...
    function editorRenderQuestion(q, topicName) {
        ...
        const optHtml = opts.map((o, i) => {
            const correct = q.a === i;
            return `<div class="eopt${correct ? ' correct' : ''}">
                <span class="eletter">${labels[i]}</span>
                <span style="flex:1">${escHtml(o)}</span>
                ${correct ? '<span class="material-icons-round" style="font-size:16px;color:#22c55e;flex-shrink:0;margin-top:1px">check_circle</span>' : ''}
            </div>`;
        }).join('');

        card.innerHTML = `
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:14px;flex-wrap:wrap">
                <span style="font-size:.72rem;color:#94a3b8;font-weight:600;letter-spacing:.06em;text-transform:uppercase;background:#f1f5f9;border:1px solid #e2e8f0;padding:3px 8px;border-radius:5px">${escHtml(q.id || '')}</span>
                <span style="font-size:.8rem;background:linear-gradient(135deg,#eff6ff,#e0e7ff);color:#3b82f6;padding:4px 12px;border-radius:99px;font-weight:700;border:1px solid #bfdbfe">${escHtml(topicName || '')}</span>
            </div>
            <p style="font-size:1.05rem;line-height:1.8;margin-bottom:18px;color:#0f172a;font-weight:500">${escHtml(q.q || '')}</p>
            <div style="display:flex;flex-direction:column;gap:1px">${optHtml}</div>
            ${q.e ? `<div class="eexplain"><span class="material-icons-round eexplain-icon">menu_book</span><span><strong>Açıklama:</strong> ${escHtml(q.e)}</span></div>` : ''}
        `;
        ...
        if (btnEdit) btnEdit.innerHTML = '<span class="material-icons-round" style="font-size:17px">edit</span> Düzenle';
    ...
    function editorToggleEdit() {
        ...
        if (btn) btn.innerHTML = window._editorState.editOpen
            ? '<span class="material-icons-round" style="font-size:17px">close</span> Kapat'
            : '<span class="material-icons-round" style="font-size:17px">edit</span> Düzenle';
    ...
    function editorSearchCard(r) {
        ...
        const optHtml = opts.map((o, i) => {
            ...
            return `<div class="eopt${correct ? ' correct' : ''}">
                <span class="eletter">${labels[i]}</span>
                <span style="flex:1">${escHtml(o)}</span>
                ${correct ? '<span class="material-icons-round" style="font-size:16px;color:#22c55e;flex-shrink:0">check_circle</span>' : ''}
            </div>`;
        }).join('');

        return `<div class="ecard" style="margin-bottom:12px">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:10px;flex-wrap:wrap">
                <span style="font-size:.72rem;color:#94a3b8;font-weight:600;letter-spacing:.06em;text-transform:uppercase;background:#f1f5f9;border:1px solid #e2e8f0;padding:3px 8px;border-radius:5px">${escHtml(r.id || '')}</span>
                <span style="font-size:.8rem;background:linear-gradient(135deg,#eff6ff,#e0e7ff);color:#3b82f6;padding:4px 12px;border-radius:99px;font-weight:700;border:1px solid #bfdbfe">${escHtml(r.topicName || r.topicId || '')}</span>
            </div>
            <p style="margin:0 0 12px;line-height:1.7;color:#0f172a;font-weight:500;font-size:1rem">${escHtml(r.q || '')}</p>
            <div style="display:flex;flex-direction:column;gap:1px">${optHtml}</div>
            ${r.e ? `<div class="eexplain"><span class="material-icons-round eexplain-icon">menu_book</span><span><strong>Açıklama:</strong> ${escHtml(r.e)}</span></div>` : ''}
        </div>`;
    }
